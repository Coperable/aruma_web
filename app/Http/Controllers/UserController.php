...
use Aruma\User;
use Aruma\Model\UserRole;
use Aruma\Model\UserCenter;
    ...
    public function assignOrganizations(Request $request) {
        ...
        if($user->hasRole('crud_user')) {
            DB::transaction(function() use ($request) {
                ...
                DB::table('organizations_users')->where('user_id', '=', $user_id)->delete();
               
                foreach($organizations as $organization) {
                    $userOrganization =  UserOrganization::firstOrCreate(array(
                        'user_id' => $user_id,
        ...
        return response()->json(['message' => 'Organizationes asignadas'], 200);
    }

    public function assignCenters(Request $request) {
        $user = User::find($request['user']['sub']);
        if($user->hasRole('crud_user')) {
            DB::transaction(function() use ($request) {
                $user_id = $request->input('userId');
                ...
                DB::table('centers_users')->where('user_id', '=', $user_id)->delete();
               
                foreach($centers as $center) {
                    $userCenter =  UserCenter::firstOrCreate(array(
                        'user_id' => $user_id,
                        'center_id' => $center['id'],
                        'admin' => true
                    ));
